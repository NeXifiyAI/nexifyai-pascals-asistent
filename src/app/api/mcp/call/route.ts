...
async function knowledgeStore(content: string, category: string, tags?: string[]) {
    ...
    const response = await fetch(`${qdrantUrl}/collections/nexify_knowledge/points`, {
      ...
      body: JSON.stringify({
        points: [{
          id: pointId,
          vector: embedding,
          payload: { content, category, tags: tags || [], timestamp: new Date().toISOString() }
        }]
      })
    });

    return { success: true, id: pointId, category };
  } catch (error: any) {
...
async function knowledgeQuery(query: string, category?: string, limit?: number) {
  ...
  try {
    ...
    const response = await fetch(`${qdrantUrl}/collections/nexify_knowledge/points/search`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'api-key': qdrantKey
...
async function registerTool(name: string, description: string, endpoint: string, method?: string, parameters?: any) {
